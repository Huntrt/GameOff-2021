...
public class Formator : MonoBehaviour
{
	...
	//Make this component into singleton
	...
	void GenerateGoals()
	{
		...
		for (int f = 0; f < goals.Count; f++)
		{
			...
			if(counter == row)
			{
				...
				if(!getWidth) {size.x = targetPos.x - offset.x; getWidth = true;HighlightColor(goals[f-1]);}
			...
			if(f == goals.Count-1) {size.y = targetPos.y; HighlightColor(goals[f]);}
		}
		...
		generated?.Invoke();
		//* An thank for helping me with formation: https://www.youtube.com/watch?v=uTBCVgrk0lY
	}

	void HighlightColor(GameObject target)
	{
		//Hightligh formation goal color
